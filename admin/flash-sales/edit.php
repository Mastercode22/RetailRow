...
$flash_sale_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
$flash_sale = null;
$errors = [];
$success_message = '';
...
if ($flash_sale_id > 0) {
    try {
        $stmt = $conn->prepare("SELECT fs.*, p.name as product_name, p.price as product_price FROM flash_sales fs LEFT JOIN products p ON fs.product_id = p.id WHERE fs.id = ?");
        $stmt->execute([$flash_sale_id]);
        $flash_sale = $stmt->fetch(PDO::FETCH_ASSOC);
    } catch (Exception $e) {
        $errors[] = "Error loading flash sale: " . $e->getMessage();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check if product already has an active flash sale (excluding current one)
    if ($product_id > 0 && empty($errors)) {
        try {
            $stmt = $conn->prepare("SELECT id FROM flash_sales WHERE product_id = ? AND is_active = 1 AND end_date > NOW() AND id != ?");
            $stmt->execute([$product_id, $flash_sale_id]);
            if ($stmt->fetch()) {
    ...
    if (empty($errors)) {
        try {
            $stmt = $conn->prepare("UPDATE flash_sales SET title = ?, product_id = ?, discount_percentage = ?, end_date = ?, is_active = ? WHERE id = ?");
            $stmt->execute([$title, $product_id, $discount_percentage, $end_date, $is_active, $flash_sale_id]);

            $success_message = "Flash sale updated successfully!";
            // Refresh flash sale data
            $flash_sale['title'] = $title;
            $flash_sale['product_id'] = $product_id;
            $flash_sale['discount_percentage'] = $discount_percentage;
            ...
            foreach ($products as $product) {
                if ($product['id'] == $product_id) {
                    $flash_sale['product_name'] = $product['name'];
                    $flash_sale['product_price'] = $product['price'];
                    break;
                }
            }
        } catch (Exception $e) {
...
<head>
    ...
    <title>Edit Flash Sale - RetailRow Admin</title>
    ...
    <div class="admin-container">
        ...
        <main class="admin-main">
            <div class="admin-header">
                <h1 class="admin-title">Edit Flash Sale</h1>
                ...
                <form method="POST">
                    <div class="form-group">
                        ...
                        <input type="text" id="title" name="title" class="form-input"
                               value="<?php echo htmlspecialchars($flash_sale['title']); ?>" required>
                    ...
                    <div class="form-row">
                            ...
                            <select id="product_id" name="product_id" class="form-select" required>
                                ...
                                <?php foreach ($products as $product): ?>
                                    <option value="<?php echo $product['id']; ?>"
                                            ...
                                            <?php echo $flash_sale['product_id'] == $product['id'] ? 'selected' : ''; ?>>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" id="discount_percentage" name="discount_percentage" class="form-input"
                                   value="<?php echo htmlspecialchars($flash_sale['discount_percentage']); ?>" min="1" max="90" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="datetime-local" id="end_date" name="end_date" class="form-input"
                               value="<?php echo htmlspecialchars(date('Y-m-d\TH:i', strtotime($flash_sale['end_date']))); ?>" required>
                    ...
                    <div class="form-group">
                        <div class="checkbox-group">
                            <input type="checkbox" id="is_active" name="is_active"
                                   <?php echo $flash_sale['is_active'] ? 'checked' : ''; ?>>
                    ...
                    <div id="productPreview" class="product-preview">
                        <div class="product-info">
                            <div>
                                <strong>Selected Product:</strong> <span id="selectedProductName"><?php echo htmlspecialchars($flash_sale['product_name'] ?? 'None'); ?></span>
                            </div>
                            <div>
                                <span class="product-price">Original: $<span id="originalPrice"><?php echo number_format($flash_sale['product_price'] ?? 0, 2); ?></span></span>
                                <span class="discounted-price">Discounted: $<span id="discountedPrice"><?php echo number_format(($flash_sale['product_price'] ?? 0) * (1 - $flash_sale['discount_percentage'] / 100), 2); ?></span></span>
                            </div>
                        </div>
                    </div>

                    <button type="submit" class="submit-btn">Update Flash Sale</button>
                </form>
            </div>
        </main>
    </div>
